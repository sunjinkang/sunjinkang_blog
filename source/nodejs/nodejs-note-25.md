...
对于 setImmediate() 和 setTimeout() 的 promise 化变体，可以使用 AbortController 来取消定时器。 当取消时，返回的 Promise 将使用 'AbortError' 拒绝。
...
新增于: v0.0.1
timeout <Timeout> | <string> | <number> 由 setInterval() 返回的 Timeout 对象、或作为字符串或数字的 Timeout 对象的原始类型。
...
新增于: v0.0.1
timeout <Timeout> | <string> | <number> 由 setTimeout() 返回的 Timeout 对象、或作为字符串或数字的 Timeout 对象的原始类型。
