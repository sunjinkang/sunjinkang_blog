...
3. 比较*oldStartId*x和*newEndIdx*指向的元素，元素不同，不可复用，什么都不用做
4. 比较*oldEndIdx*x和*newStartIdx*指向的元素，元素相同，可复用，需要移动元素
...
```
上面的代码保证了非理想情况下的双端对比能够进行对比处理，但是我们在实际应用双端对比进行处理时，上面的while条件往往会产生问题，如果新旧数组的长度不一致，那么当短数组对比结束时，长数组还没对比完成，但上面的代码根据while条件实际上已经不能再继续执行，这时候就需要做针对处理，保证对比的正确性
