...
// 打印错误信息
const printErrorMessage = (msg: string) => {
  console.log(chalk.red(msg));
};

// 打印成功信息
const printSuccessMessage = (msg: string) => {
...
// 检查文件夹是否存在
const isFolderExist = (folderPath: string) => {
  try {
...
// 检查文件是否存在
const isFileExist = (targetPath: string) => {
  try {
...
// 新建文件夹
const createDirectory = async (targetDir: string) => {
  try {
...
// 向文件中新增内容
const insertContentIntoFile = (targetPath: string, newContent: string) => {
